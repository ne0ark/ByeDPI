...
RUN apk add --no-cache curl tar \
 && TAG=$(curl -sfL https://api.github.com/repos/hufrea/byedpi/releases/latest | grep -m1 '"tag_name"' | sed 's/.*"tag_name": *"\([^"]*\)".*/\1/') \
 && curl -sfL "https://github.com/hufrea/byedpi/releases/download/${TAG}/byedpi-${TAG#v}-${BYEDPI_ARCH}.tar.gz" -o /tmp/byedpi.tar.gz \
